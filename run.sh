...
# Working example that writes result column to file
echo "Working example of print_fires.py that writes result list to file:"
python3 src/print_fires.py \
    --file-name "$file_name" --country-column $query_column \
    --country "$query_value" --write-file "test_fires_output.txt"

# Working example that plots histogram of file with result column
python src/plot_fires.py --file-name test_fires_result_col0.txt \
    --title "title" --x-label "x" --y-label "y"